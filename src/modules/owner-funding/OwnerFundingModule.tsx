...
export function OwnerFundingModule({ role, currentUserId }: Props) {
  ...
  useEffect(() => { getOwnerFunding().then((rows) => setFunding(sortByEnteredDateDesc(rows, (item) => item.funding_date, (item) => item.created_at))).catch((error: Error) => setError(error.message)).finally(() => setLoading(false)) }, [])
  ...
  async function addFunding(event: React.FormEvent) {
    ...
    try { const item = await createOwnerFunding(form); setFunding((current) => sortByEnteredDateDesc([...current, item], (entry) => entry.funding_date, (entry) => entry.created_at)); setModalOpen(false); setForm(createEmptyFunding()) }
