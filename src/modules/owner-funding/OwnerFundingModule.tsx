...
export function OwnerFundingModule({ role, currentUserId }: Props) {
  ...
  function exportCsv() {
    const rows = [['Date', 'Owner', 'Description', 'Direction', 'Created by', 'Amount'], ...filtered.map((item) => [item.funding_date, item.owner_name, item.description ?? '', item.direction, item.created_by_email, String(item.amount)])]
