...
export function ExpensesTable({ expenses, loading, canDelete, onDelete }: Props) {
  ...
  return <div className="table-wrap"><table className="expenses-table">
    <thead><tr><th>Date</th><th>Merchant / description</th><th>Quantity</th><th className="amount">Unit price</th><th>Category</th><th>Project</th><th>Payment</th><th>Cash account</th><th>Status</th><th>Created by</th><th className="amount">Amount</th><th /></tr></thead>
    <tbody>{loading ? <tr><td colSpan={12} className="empty">Loading expenses…</td></tr> : expenses.length === 0 ? <tr><td colSpan={12} className="empty">No expenses match your filters.</td></tr> : expenses.map((expense) => <tr key={expense.id}>
    ...
    {!loading && <tfoot><tr>
      <td colSpan={10} className="total-label">Total for displayed expenses</td>
