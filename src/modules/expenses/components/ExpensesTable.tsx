...
export function ExpensesTable({ expenses, loading, onDelete }: Props) {
  ...
  return <div className="table-wrap"><table>
    <thead><tr><th>Date</th><th>Merchant / description</th><th>Category</th><th>Payment</th><th>Status</th><th className="amount">Amount</th><th /></tr></thead>
    <tbody>{loading ? <tr><td colSpan={7} className="empty">Loading expenses…</td></tr> : expenses.length === 0 ? <tr><td colSpan={7} className="empty">No expenses match your filters.</td></tr> : expenses.map((expense) => <tr key={expense.id}>
      ...
      <td><button className="icon-button delete" title="Delete expense" onClick={() => onDelete(expense)}><Trash2 size={15} /></button></td>
    </tr>)}</tbody>
    {!loading && <tfoot><tr>
      <td colSpan={5} className="total-label">Total for displayed expenses</td>
