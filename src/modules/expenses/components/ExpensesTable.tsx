...
export function ExpensesTable({ expenses, loading, canDelete, onDelete }: Props) {
  ...
  return <div className="table-wrap"><table className="expenses-table">
    <thead><tr><th>Date</th><th>Merchant / description</th><th>Quantity</th><th className="amount">Unit price</th><th>Category</th><th>Project</th><th>Truck</th><th>Payment</th><th>Cash account</th><th>Status</th><th>Created by</th><th className="amount">Amount</th><th /></tr></thead>
    <tbody>{loading ? <tr><td colSpan={13} className="empty">Loading expenses…</td></tr> : expenses.length === 0 ? <tr><td colSpan={13} className="empty">No expenses match your filters.</td></tr> : expenses.map((expense) => <tr key={expense.id}>
      ...
      <td>{expense.project_name ?? 'General'}</td>
      <td>{expense.truck_name ? <div className="expense-truck"><strong>{expense.truck_name}</strong><span>{expense.truck_registration_number}{expense.fuel_tank_reading_liters !== null ? ` · Tank ${expense.fuel_tank_reading_liters} L` : ''}</span></div> : '—'}</td>
      <td>{expense.payment_method === 'Fuel card' ? <div className="expense-payment"><strong>Fuel card</strong><span>{expense.fuel_card_name} · {expense.fuel_card_number}</span></div> : expense.payment_method}</td><td>{expense.cash_account_name ?? '—'}</td><td><span className={`status ${expense.status}`}><i />{expense.status}</span></td>
    ...
    {!loading && <tfoot><tr>
      <td colSpan={11} className="total-label">Total for displayed expenses</td>
