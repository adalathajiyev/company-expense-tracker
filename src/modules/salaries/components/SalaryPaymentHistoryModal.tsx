...
import { formatDate, formatMonth } from '../../../lib/businessDate'
...
export function SalaryPaymentHistoryModal({ salary, onClose, onDelete }: Props) {
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="modal payment-history-modal">
    ...
    <div className="payment-history-list">{salary.payments.length === 0 ? <div className="payment-history-empty">No salary payments are applied to this month.</div> : salary.payments.map((payment) => <div className="payment-history-item" key={`${payment.id}-${salary.id}`}>
      <div><strong>{currency.format(Number(payment.applied_amount))}</strong><span>{formatDate(payment.payment_date)} · {paymentTypeLabels[payment.payment_type]}</span>{payment.applied_amount !== payment.amount && <small>From a {currency.format(Number(payment.amount))} payment</small>}{payment.origin_salary_month !== salary.salary_month && <small>Carried from {formatMonth(payment.origin_salary_month)}</small>}{payment.note && <small>{payment.note}</small>}</div>
